...
const configureRequest = config => {
    ...
    if (innerConfig && innerConfig.data && !innerConfig?.url.includes('localhost')) {
        ...
        const dataParser =
            innerConfig.data && typeof innerConfig.data === 'string' ? JSON.parse(innerConfig.data) : innerConfig.data;
        innerConfig.data = LowUp.up(dataParser, el => el instanceof FormData);
    }
    if (isValidXss(url)) throw new Error(`URL contains XSS injection attempt at resolveURL\nURL: ${url}`);
