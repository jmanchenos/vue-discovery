...
function report(error) {
    ...
    if (config?.retry) {
        uid = config.retry.uid || response?.data?.uid || response?.headers?.traceid || `front-${generateUid()}`;
...
function findByStatusCode(err) {
    ...
    if (status) {
        ...
        found = codesNoResponse.concat(statusCodesNoResponse).find(statusCode => statusCode.error === err.code);
    }
