...
const comprobarDeshabilitado = (dataCy, data) => {
    cy.get(`[data-cy=${dataCy}]`).then(item => {
        ...
        if (!isDisabled) {
            ...
            cy.get(`[data-cy=${dataCy}]`).type(data).should('have.value', data);
        }
    });
};
Cypress.Commands.add(
    'rellenarDatosNig',
    function (coPobNig, tiOrgNig, ordOrgNig, anAsuNig, nuAsuNig, indicadorNigValido) {
        ...
        cy.get('[data-cy="coPobNig"]').type(coPobNig).should('have.value', coPobNig);
        cy.wait(500);
        cy.get('[data-cy="tiOrgNig"]').type(tiOrgNig).should('be.visible').and('have.value', tiOrgNig);
        cy.get('[data-cy="ordOrgNig"]').type(ordOrgNig).should('be.visible').and('have.value', ordOrgNig);
        cy.get('[data-cy="anAsuNig"]').type(anAsuNig).should('be.visible').and('have.value', anAsuNig);
        // al hacer el blur lanza las acciones del nig
        cy.get('[data-cy="nuAsuNig"]').type(nuAsuNig).blur();
...
Cypress.Commands.add('seleccionDelito', delito => {
    cy.get('[data-cy="codigoDelito"]').type(delito).should('be.visible').and('have.value', delito);
});
...
Cypress.Commands.add('aniadirDelitos', function (delito) {
    ...
    materiasHeredadasNig.forEach((item, index) => {
        ...
        if (!item.seleccionable) {
            cy.get(`[data-row-selected=${indexArray}]`).find('i').first().should('have.class', 'text-negative');
        }
    });
    ...
    cy.get('[data-cy="codigoDelito"]').type(delito, { force: true }).should('be.visible').and('have.value', delito);
});
...
Cypress.Commands.add('resgistrarSinDelito', function (claseReparto) {
    cy.get('[data-cy="claseReparto"]').type(claseReparto).should('be.visible').and('have.value', claseReparto);
    cy.get('[data-cy="btnReg"]').click({ force: true });
    cy.wait(1000);
    cy.get('[data-cy="codigoDelito"]').parents('label').should('have.class', 'q-field--error');
});
...
Cypress.Commands.add('rellenarDelito', (delito, claseRegistro, claseReparto) => {
    ...
    cy.get('[data-cy="codigoClaseRegistro"]').type(claseRegistro).should('be.visible').and('have.value', claseRegistro);
    cy.wait(1000);
    cy.get('[data-cy="clasesRepartoAsocCancelar"]').click({ force: true });
    ...
    cy.wait(1000);
    cy.get('[data-cy="codigoClaseRegistro"]').focus().tab();
    cy.wait(1000);
    cy.compruebaMuestraMensajeNotificacion('No existe relación entre la clase de registro y la clase de reparto');
});
...
Cypress.Commands.add('abandonaPantallaAndReturn', () => {
    ...
    cy.get('[data-cy="numero"]').then($span => {
        ...
        if (materiasBorradasHeredadasNig) expect(lengthMateriasInicioNig).not.to.equal(lengthMateriasDespuesBorrado);
...
Cypress.Commands.add('comprobarNoHayMateriasPrincipales', function () {
    materiasHeredadasNig.forEach((item, index) => {
        if (!item.principal && item.eliminable)
            cy.get(`[data-cy="materiasTable${index}icon-borrar"]`).should('not.have.css', 'display', 'none');
        else cy.get(`[data-cy="materiasTable${index}icon-borrar"]`).should('have.css', 'display', 'none');
    });
});
...
Cypress.Commands.add('rellenarDelitoCompruebaClaseRepartoDestino', delito => {
    ...
    cy.wait('@comboDestino').then(xhr => {
        ...
        if (destino?.content) setOrganoDestinoDefecto = destino.content?.organos.some(item => item.defecto === true);
        cy.wait(1000);
...
Cypress.Commands.add('rellenarDelitoClaseReparto', (delito1, delito2, delito3, claseReparto) => {
    ...
    cy.wait(1000);
    cy.get('[data-cy="clasesRepartoAsoc"]').find('input').eq(0).click({ force: true });
...
Cypress.Commands.add('revisionDelitoConClaseRepartoAsociada', (delito, claseReparto, claseRegistro) => {
    cy.intercept({
        method: 'GET',
...
Cypress.Commands.add('addDelitoMateria', (delito, tipificaciones) => {
    ...
    selectDelito.find('[role="checkbox"]').first().click({ force: true });
    cy.wait(1000);
    const itemSeletectedTable = cy.get('[data-cy="materiasTable"]').find('[class="q-virtual-scroll__content"]');
...
Cypress.Commands.add('procedimientoOrigen', (poblacion, tipoOrgano, numeroOrgano, claseProcedimiento, number, year) => {
    cy.get('[data-cy="codigoPoblacionSelectorOrgano"]').type(poblacion).should('have.value', poblacion);
    cy.wait(1000);
    cy.get('[data-cy="tipoOrganoSelectorOrgano"]').type(tipoOrgano).should('be.visible').and('have.value', tipoOrgano);
    cy.get('[data-cy="numeroOrganoSelectorOrgano"]')
        ...
        .then(() => {
            ...
            cy.get('[data-cy="numeroAnyoInput1"]').type(number, { force: true }).should('have.value', number);
            cy.get('[data-cy="numeroAnyoInput2"]').type(year, { force: true }).should('have.value', year);
        });

    cy.intercept({
        method: 'GET',
        url: '/mtm-back-consultas-general/api/no-pragma/procedimiento/mismoNIG?nig=1607851220180000404&soloContador=true',
    }).as('procedimientosMismoNig');
