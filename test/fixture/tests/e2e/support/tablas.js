...
Cypress.Commands.add(
  'validaOrdenTablaPorNumColumna',
  (tabla, indexColumna, orderAsc = true, sortFn = '') => {
    ...
    const sortFnText2 = (a, b) =>
      [numerosAfter(a), numerosAfter(b)].sort()[0] === numerosAfter(a) ? -1 : 1; //
    let sortFnFinal;
    ...
    cy.findByTestId(tabla)
      ...
      .then(tds => {
        ...
        expect($valores).to.deep.equal(
          orderAsc ? valoresOrdenados : valoresOrdenados.reverse()
        );
      });
  }
);
 ...
 */
Cypress.Commands.add(
  'validarOrdenacionColumna',
  (tabla, nombreColumna, ordenable = true, sortFn = '') => {
    cy.findByTestId(tabla)
...
Cypress.Commands.add('validarRegistroEsSeleccionado', { prevSubject: true }, (subject, checkState, ...args) => {
  args.forEach(arg => {
    if (typeof (arg) === 'number') {
      cy.get(subject).find(`tr.q-tr[data-row-selected=${arg}]`).should('exist').and(checkState ? 'have.class' : 'not.have.class', 'selected');
    } else if (typeof (arg) === 'string') {
      cy.get(subject).find(`tr.q-tr[data-row-selected]`)
 ...
 * */
Cypress.Commands.add('findTableIcon', { prevSubject: true },
  (subject, icon,  { actionIcon = true, filter = '', row = 'first' } = {}  ) => {
    ...
    Cypress.log({
      ...
      message:
        $el.length > 0
          ? `Icono ${icon} encontrado en la fila ${numReg}`
          : `No se ha encontrado icono ${icon}`,
      consoleProps: () => ({
        subject,
...
Cypress.Commands.add('findTableIndexByFilter', { prevSubject: true }, (subject, columnId, value) => {
  ...
  const findIndex =  (tableSubject, lastIndexTreated = 0) => {
    ...
    if (filteredCells.length) {
      ...
      cy.wrap(cells.last()[0], {log:false}).scrollIntoView().should(el => {
  ...
  let totalRegs = 0;
  cy.wrap(subject, { log: false }).getNumRegistros().then(numReg => {
    totalRegs = numReg;
    findIndex(subject, 0);
...
Cypress.Commands.add('scrollIntoIndex', { prevSubject: true }, (subject, index) => {
  ...
  const findAndScroll = () => {
    ...
    if (lastIndex <= index) {
      cy.wrap(regs.last()[0], {log:false}).scrollIntoView().should(el => {
