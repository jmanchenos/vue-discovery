...
import * as completionProvider from '@/completionProvider.mjs';
import * as hoverProvider from '@/hoverProvider.mjs';
import * as definitionProvider from '@/definitionProvider.mjs';
import * as commandProvider from '@/commandProvider.mjs';
import * as contextMenuProvider from '@/contextMenuProvider.mjs';
  ...
  try {
    ...
    if (
      ...
      await Promise.all([
        ...
        utils
          .getFilesByExtension('vue', 'registeredDirectory')
          .then(res => registeredFiles.push(...res)),
      ]).then(() => {
      ...
      Promise.all(preformattedData.vueFiles.map(utils.getComponentTuple)).then(result =>
        config.setVueFiles(result || [])
      );
      Promise.all(preformattedData.vueRegisteredFiles.map(utils.getComponentTuple)).then(result =>
    ...
    console.error('Error en setupCurrentWorkspace:', error);
...
export async function activate(ctx = null) {
  try {
    ...
    config.outputChannel.appendLine(
      `Ha tardado ${endTimestamp - initTimestamp} ms en cargar la extensión`
    );
  } catch (error) {
