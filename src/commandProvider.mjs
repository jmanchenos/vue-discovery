 ...
 * @returns {string} importPath
 * @throws {Error} If there is an error while generating the import path.
 * @description
 * Generates the import path for a Vue component based on the provided item and file name.
 * The function checks if the item has an alias and constructs the import path accordingly.
 * If an alias is found, it replaces the alias path with its value. Otherwise, it generates a relative path
 * using the file name and the current root path.
 * The generated import path is used to import the Vue component in the script section of the file.
 */

function getImportPath(item, fileName) {
  try {
    const fileWithoutRootPath = item.filePath.replace(`${utils.getRootPath()}/`, '');
    ...
    console.error('Error en getImportPath:', error);
  }
}
/**
 ...
 * @param {String} fileName
 */
async function insertImport(item, fileName) {
  try {
    const componentName = utils.pascalCase(item.componentName || fileName);
    if (!utils.isComponentRegistered(componentName)) {
      const document = utils.getDocument();
      const text = utils.getDocumentText();
      const match = /<script/.exec(text);
      const importPath = getImportPath(item, fileName);
      if (text.indexOf(`import ${componentName} from '${importPath}`) === -1) {
    ...
    console.error('Error en insertImport:', error);
  }
}
...
async function insertComponents(text, componentName, eol) {
  try {
    ...
    console.error('Error en insertComponent:', error);
  }
}
...
async function insertInExistingComponents(match, componentString, eol) {
  try {
    ...
    console.error('Error en insertInExistingComponents:', error);
  }
}
/**
...
async function insertComponent(componentName) {
  try {
    ...
    const testYaRegistrado = new RegExp(`components\\s*:\\s*{[^}]*${componentString}\\W`, 'g').test(
      text
    );
    if (testYaRegistrado) {
      return;
 ...
 * @param {String} fileName
 */
async function addComponentSnippet(item, fileName) {
  try {
    const requiredProps = await utils.retrieveRequirePropsFromFile(item);
    const hasSlots = await utils.retrieveHasSlots(item);
    ...
    outputChannel.appendLine(`No se ha podido insertar snippet en template para :  ${fileName}`);
  }
}
 ...
 *
 * @param {Object} item - The item of the file to import.
 * @param {string} fileName - The name of the file.
 * @returns {Promise<void>} - A promise that resolves when the import process is complete.
 */
const importFile = commands.registerCommand(
  ...
  async (item, fileName) => {
    try {
      ...
      await insertImport(item, fileName);
      await insertComponent(componentName);
      await addComponentSnippet(item, componentName);
    } catch (error) {
      outputChannel.appendLine(`Error en importFile: ${error.message}`);
    }
  }
);
 ...
 */
const setConfigOption = commands.registerCommand(
 ...
 */
const deleteNodeModules = commands.registerCommand(
  'VueDiscoveryMTM.deleteNodeModules',
  async uri => {
    if (uri?.fsPath) {
 ...
 */
const createTestUnitFile = commands.registerCommand(
  'VueDiscoveryMTM.createTestUnitFile',
  async uri => {
